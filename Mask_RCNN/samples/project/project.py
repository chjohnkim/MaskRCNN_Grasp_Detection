...
PROJECT_WEIGHTS_PATH = os.path.join(ROOT_DIR, "mask_rcnn_project.h5")
...
class ProjectConfig(Config):
    ...
    NUM_CLASSES = 1 + 4  # Background + gripper + cube + cylinder + grasp
...
class ProjectDataset(utils.Dataset):
    ...
    def load_project(self, dataset_dir, subset):
        """Load a subset of the Project dataset.
        dataset_dir: Root directory of the dataset.
        subset: Subset to load: train or val
        """
        # Add classes. We have only one class to add.
        self.add_class("project", 1, "gripper")
        self.add_class("project", 2, "cube")
        self.add_class("project", 3, "cylinder")
        self.add_class("project", 4, "grasp")
        ...
        # Add a region attribute face:gripper/cube/cylinder
            faces = [t['region_attributes'] for t in a['regions'].values()]
            for i, p in enumerate(faces):
                if(faces[i]['name'] == "gripper"):
                    faces[i]['name'] = 1
                elif(faces[i]['name'] == "cube"):
                    faces[i]['name'] = 2
                elif(faces[i]['name'] == "cylinder"):
                    faces[i]['name'] = 3
                elif(faces[i]['name'] == "grasp"):
                    faces[i]['name'] = 4
            class_ids = [int(n['name']) for n in faces]
    ...
    def load_mask(self, image_id):
       ...
       Returns:
        ...
        """
        # If not a project dataset image, delegate to parent class.
        image_info = self.image_info[image_id]
        if image_info["source"] != "project":
        ...
        #print(info["faces"])
        #print(info["faces"][0]['face'])
        #print(info["polygons"])
        #for t, q in enumerate(info["faces"]):
        #    print(info["faces"][t]['face'])
...
#        # [height, width, instance_count]
#        info = self.image_info[image_id]
#        face = ["" for face in range(len(info["faces"]))]
#        for i, p in enumerate(info["faces"]):
#            face[i] = info["faces"][i]['face']
#        return face
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Train Mask R-CNN to detect projects.')
    ...
    elif args.weights.lower() == "project":
        # Start from ImageNet trained weights
        weights_path = PROJECT_WEIGHTS_PATH
    else:
